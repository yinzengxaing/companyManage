...
public class SessionFilter implements Filter {
	...
	//
	private static final String HYT_HYT = "/views";
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		Map<String, Object> user = (Map<String, Object>) session.getAttribute("admTsyUser");

		if (user == null && url.equals("/companyManage/post/CompanyNewsController/getNewsList")){
			servletResponse.sendRedirect(LOGIN_PAGE);
		}else if (url.indexOf(HYT_HYT) < 0 && url.indexOf(ADM_HYT) < 0) {
			...
			if (url.equals("/companyManage/html/views/manage.html")) {
				 ...
				 * System.out.println("html"); chain.doFilter(request,
				 * response); } else if (url.indexOf(ADM_HYT) != -1) {//
				 * post请求通过 System.out.println("post"); chain.doFilter(request,
				 * response); } else if (url.indexOf(ADM_GATEWAY) != -1) {//
				 * gateway请求通过 System.out.println("getaway");
				 * chain.doFilter(request, response); } else if
				 * (url.indexOf(HYT_HYT_MENU) != -1) {// menu请求通过
				 * System.out.println("menu"); chain.doFilter(request,
				 * response); } else { }
				 */
			}
		}
